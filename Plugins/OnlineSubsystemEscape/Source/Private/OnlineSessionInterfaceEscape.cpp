...
#include "NboSerializerEscape.h"



FOnlineSessionInfoEscape::FOnlineSessionInfoEscape() :
	HostAddr(NULL),
	SessionId(TEXT("INVALID"))
{
}

void FOnlineSessionInfoEscape::Init(const FOnlineSubsystemEscape& Subsystem)
{
	...
	SessionId = FUniqueNetIdEscape(OwnerGuid.ToString());
}

/**
 *	Async task for ending a Escape online session
 */
class FOnlineAsyncTaskEscapeEndSession : public FOnlineAsyncTaskBasic<FOnlineSubsystemEscape>
{
private:
...
public:
	FOnlineAsyncTaskEscapeEndSession(class FOnlineSubsystemEscape* InSubsystem, FName InSessionName) :
		FOnlineAsyncTaskBasic(InSubsystem),
		SessionName(InSessionName)
	{
	}

	~FOnlineAsyncTaskEscapeEndSession()
	{
	}
	...
	{
		return FString::Printf(TEXT("FOnlineAsyncTaskEscapeEndSession bWasSuccessful: %d SessionName: %s"), WasSuccessful(), *SessionName.ToString());
	}
...
/**
 *	Async task for destroying a Escape online session
 */
class FOnlineAsyncTaskEscapeDestroySession : public FOnlineAsyncTaskBasic<FOnlineSubsystemEscape>
{
private:
	/** Name of session ending */
	FName SessionName;
...
public:
	FOnlineAsyncTaskEscapeDestroySession(class FOnlineSubsystemEscape* InSubsystem, FName InSessionName) :
		FOnlineAsyncTaskBasic(InSubsystem),
		SessionName(InSessionName)
	{
	}

	~FOnlineAsyncTaskEscapeDestroySession()
	{
	}
	...
	virtual FString ToString() const override
	{
		return FString::Printf(TEXT("FOnlineAsyncTaskEscapeDestroySession bWasSuccessful: %d SessionName: %s"), WasSuccessful(), *SessionName.ToString());
	}
...
bool FOnlineSessionEscape::CreateSession(int32 HostingPlayerNum, FName SessionName, const FOnlineSessionSettings& NewSessionSettings)
{
	...
	if (Session == NULL)
	{
		...
		IOnlineIdentityPtr Identity = EscapeSubsystem->GetIdentityInterface();
		if (Identity.IsValid())
		{
		...
		{
			Session->OwningUserId = MakeShareable(new FUniqueNetIdEscape(FString::Printf(TEXT("%d"), HostingPlayerNum)));
			Session->OwningUserName = FString(TEXT("EscapeUser"));
		}
		...
		FOnlineSessionInfoEscape* NewSessionInfo = new FOnlineSessionInfoEscape();
		NewSessionInfo->Init(*EscapeSubsystem);
...
bool FOnlineSessionEscape::CreateSession(const FUniqueNetId& HostingPlayerId, FName SessionName, const FOnlineSessionSettings& NewSessionSettings)
{
	// todo: use proper	HostingPlayerId
	return CreateSession(0, SessionName, NewSessionSettings);
}

bool FOnlineSessionEscape::IsSessionJoinable(const FNamedOnlineSession& Session) const
{
...
bool FOnlineSessionEscape::StartSession(FName SessionName)
{
	uint32 Result = E_FAIL;
...
bool FOnlineSessionEscape::UpdateSession(FName SessionName, FOnlineSessionSettings& UpdatedSessionSettings, bool bShouldRefreshOnlineData)
{
...
bool FOnlineSessionEscape::EndSession(FName SessionName)
{
	uint32 Result = E_FAIL;
...
bool FOnlineSessionEscape::DestroySession(FName SessionName, const FOnDestroySessionCompleteDelegate& CompletionDelegate)
{
	uint32 Result = E_FAIL;
...
bool FOnlineSessionEscape::IsPlayerInSession(FName SessionName, const FUniqueNetId& UniqueId)
{
	return IsPlayerInSessionImpl(this, SessionName, UniqueId);
}

bool FOnlineSessionEscape::StartMatchmaking(const TArray< TSharedRef<const FUniqueNetId> >& LocalPlayers, FName SessionName, const FOnlineSessionSettings& NewSessionSettings, TSharedRef<FOnlineSessionSearch>& SearchSettings)
{
...
bool FOnlineSessionEscape::CancelMatchmaking(int32 SearchingPlayerNum, FName SessionName)
{
...
bool FOnlineSessionEscape::CancelMatchmaking(const FUniqueNetId& SearchingPlayerId, FName SessionName)
{
	UE_LOG(LogOnline, Warning, TEXT("CancelMatchmaking is not supported on this platform. Use CancelFindSessions."));
	TriggerOnCancelMatchmakingCompleteDelegates(SessionName, false);
	return false;
...
bool FOnlineSessionEscape::FindSessions(int32 SearchingPlayerNum, const TSharedRef<FOnlineSessionSearch>& SearchSettings)
{
...
bool FOnlineSessionEscape::FindSessions(const FUniqueNetId& SearchingPlayerId, const TSharedRef<FOnlineSessionSearch>& SearchSettings)
{
...
bool FOnlineSessionEscape::FindSessionById(const FUniqueNetId& SearchingUserId, const FUniqueNetId& SessionId, const FUniqueNetId& FriendId, const FOnSingleSessionResultCompleteDelegate& CompletionDelegates)
{
...
bool FOnlineSessionEscape::CancelFindSessions()
{
	uint32 Return = E_FAIL;
...
bool FOnlineSessionEscape::JoinSession(int32 PlayerNum, FName SessionName, const FOnlineSessionSearchResult& DesiredSession)
{
	uint32 Return = E_FAIL;
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
...
bool FOnlineSessionEscape::JoinSession(const FUniqueNetId& PlayerId, FName SessionName, const FOnlineSessionSearchResult& DesiredSession)
{
	// Assuming player 0 should be OK here
	return JoinSession(0, SessionName, DesiredSession);
}

bool FOnlineSessionEscape::FindFriendSession(int32 LocalUserNum, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Escape subsystem
...
bool FOnlineSessionEscape::FindFriendSession(const FUniqueNetId& LocalUserId, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Escape subsystem
	TArray<FOnlineSessionSearchResult> EmptySearchResult;
...
bool FOnlineSessionEscape::FindFriendSession(const FUniqueNetId& LocalUserId, const TArray<TSharedRef<const FUniqueNetId>>& FriendList)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Escape subsystem
	TArray<FOnlineSessionSearchResult> EmptySearchResult;
	TriggerOnFindFriendSessionCompleteDelegates(0, false, EmptySearchResult);
...
bool FOnlineSessionEscape::SendSessionInviteToFriend(int32 LocalUserNum, FName SessionName, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Escape subsystem
	return false;
};

bool FOnlineSessionEscape::SendSessionInviteToFriend(const FUniqueNetId& LocalUserId, FName SessionName, const FUniqueNetId& Friend)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Escape subsystem
	return false;
}

bool FOnlineSessionEscape::SendSessionInviteToFriends(int32 LocalUserNum, FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Friends)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Escape subsystem
	return false;
};

bool FOnlineSessionEscape::SendSessionInviteToFriends(const FUniqueNetId& LocalUserId, FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Friends)
{
	// this function has to exist due to interface definition, but it does not have a meaningful implementation in Escape subsystem
	return false;
}
...
bool FOnlineSessionEscape::PingSearchResults(const FOnlineSessionSearchResult& SearchResult)
{
	return false;
}

/** Get a resolved connection string from a session info */
static bool GetConnectStringFromSessionInfo(TSharedPtr<FOnlineSessionInfoEscape>& SessionInfo, FString& ConnectInfo, int32 PortOverride=0)
{
	bool bSuccess = false;
...
bool FOnlineSessionEscape::GetResolvedConnectString(FName SessionName, FString& ConnectInfo, FName PortType)
{
	bool bSuccess = false;
	// Find the session
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
	if (Session != NULL)
	{
		TSharedPtr<FOnlineSessionInfoEscape> SessionInfo = StaticCastSharedPtr<FOnlineSessionInfoEscape>(Session->SessionInfo);
...
bool FOnlineSessionEscape::GetResolvedConnectString(const FOnlineSessionSearchResult& SearchResult, FName PortType, FString& ConnectInfo)
{
	bool bSuccess = false;
	if (SearchResult.Session.SessionInfo.IsValid())
	{
		TSharedPtr<FOnlineSessionInfoEscape> SessionInfo = StaticCastSharedPtr<FOnlineSessionInfoEscape>(SearchResult.Session.SessionInfo);
...
FOnlineSessionSettings* FOnlineSessionEscape::GetSessionSettings(FName SessionName) 
{
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
	if (Session)
	{
...
void FOnlineSessionEscape::RegisterLocalPlayers(FNamedOnlineSession* Session)
{
	if (!EscapeSubsystem->IsDedicated())
	{
		IOnlineVoicePtr VoiceInt = EscapeSubsystem->GetVoiceInterface();
		if (VoiceInt.IsValid())
		{
...
void FOnlineSessionEscape::RegisterVoice(const FUniqueNetId& PlayerId)
{
	IOnlineVoicePtr VoiceInt = EscapeSubsystem->GetVoiceInterface();
	if (VoiceInt.IsValid())
	{
		if (!EscapeSubsystem->IsLocalPlayer(PlayerId))
		{
...
void FOnlineSessionEscape::UnregisterVoice(const FUniqueNetId& PlayerId)
{
	IOnlineVoicePtr VoiceInt = EscapeSubsystem->GetVoiceInterface();
	if (VoiceInt.IsValid())
	{
...
bool FOnlineSessionEscape::RegisterPlayer(FName SessionName, const FUniqueNetId& PlayerId, bool bWasInvited)
{
	TArray< TSharedRef<const FUniqueNetId> > Players;
	Players.Add(MakeShareable(new FUniqueNetIdEscape(PlayerId)));
	return RegisterPlayers(SessionName, Players, bWasInvited);
}

bool FOnlineSessionEscape::RegisterPlayers(FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Players, bool bWasInvited)
{
	bool bSuccess = false;
	FNamedOnlineSession* Session = GetNamedSession(SessionName);
	if (Session)
...
bool FOnlineSessionEscape::UnregisterPlayer(FName SessionName, const FUniqueNetId& PlayerId)
{
	TArray< TSharedRef<const FUniqueNetId> > Players;
	Players.Add(MakeShareable(new FUniqueNetIdEscape(PlayerId)));
...
bool FOnlineSessionEscape::UnregisterPlayers(FName SessionName, const TArray< TSharedRef<const FUniqueNetId> >& Players)
{
	bool bSuccess = true;
...
void FOnlineSessionEscape::Tick(float DeltaTime)
{

}

void FOnlineSessionEscape::AppendSessionToPacket(FNboSerializeToBufferEscape& Packet, FOnlineSession* Session)
{
	/** Owner of the session */
	Packet << *StaticCastSharedPtr<const FUniqueNetIdEscape>(Session->OwningUserId)
		<< Session->OwningUserName
		<< Session->NumOpenPrivateConnections
		<< Session->NumOpenPublicConnections;

	// Try to get the actual port the netdriver is using
	SetPortFromNetDriver(*EscapeSubsystem, Session->SessionInfo);

	// Write host info (host addr, session id, and key)
	Packet << *StaticCastSharedPtr<FOnlineSessionInfoEscape>(Session->SessionInfo);

	// Now append per game settings
	AppendSessionSettingsToPacket(Packet, &Session->SessionSettings);
}

void FOnlineSessionEscape::AppendSessionSettingsToPacket(FNboSerializeToBufferEscape& Packet, FOnlineSessionSettings* SessionSettings)
{
#if DEBUG_LAN_BEACON
...
void FOnlineSessionEscape::OnValidQueryPacketReceived(uint8* PacketData, int32 PacketLength, uint64 ClientNonce)
{
	...
	for (int32 SessionIndex = 0; SessionIndex < Sessions.Num(); SessionIndex++)
	{
		...
		{
			FNboSerializeToBufferEscape Packet(LAN_BEACON_MAX_PACKET_SIZE);
...
void FOnlineSessionEscape::ReadSessionFromPacket(FNboSerializeFromBufferEscape& Packet, FOnlineSession* Session)
{
#if DEBUG_LAN_BEACON
	...
	/** Owner of the session */
	FUniqueNetIdEscape* UniqueId = new FUniqueNetIdEscape;
	Packet >> *UniqueId
		>> Session->OwningUserName
		>> Session->NumOpenPrivateConnections
		>> Session->NumOpenPublicConnections;
	...
	FOnlineSessionInfoEscape* EscapeSessionInfo = new FOnlineSessionInfoEscape();
	EscapeSessionInfo->HostAddr = ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM)->CreateInternetAddr();
	Packet >> *EscapeSessionInfo;
	Session->SessionInfo = MakeShareable(EscapeSessionInfo);

	// Read any per object data using the server object
	ReadSettingsFromPacket(Packet, Session->SessionSettings);
}

void FOnlineSessionEscape::ReadSettingsFromPacket(FNboSerializeFromBufferEscape& Packet, FOnlineSessionSettings& SessionSettings)
{
#if DEBUG_LAN_BEACON
...
void FOnlineSessionEscape::OnValidResponsePacketReceived(uint8* PacketData, int32 PacketLength)
{
	...
	if (CurrentSessionSearch.IsValid())
	{
		...
		FNboSerializeFromBufferEscape Packet(PacketData, PacketLength);
...
int32 FOnlineSessionEscape::GetNumSessions()
{
	FScopeLock ScopeLock(&SessionLock);
	return Sessions.Num();
}

void FOnlineSessionEscape::DumpSessionState()
{
	FScopeLock ScopeLock(&SessionLock);
...
void FOnlineSessionEscape::RegisterLocalPlayer(const FUniqueNetId& PlayerId, FName SessionName, const FOnRegisterLocalPlayerCompleteDelegate& Delegate)
{
	Delegate.ExecuteIfBound(PlayerId, EOnJoinSessionCompleteResult::Success);
}

void FOnlineSessionEscape::UnregisterLocalPlayer(const FUniqueNetId& PlayerId, FName SessionName, const FOnUnregisterLocalPlayerCompleteDelegate& Delegate)
{
	Delegate.ExecuteIfBound(PlayerId, true);
}

void FOnlineSessionEscape::SetPortFromNetDriver(const FOnlineSubsystemEscape& Subsystem, const TSharedPtr<FOnlineSessionInfo>& SessionInfo)
{
	auto NetDriverPort = GetPortFromNetDriver(Subsystem.GetInstanceName());
	auto SessionInfoEscape = StaticCastSharedPtr<FOnlineSessionInfoEscape>(SessionInfo);
	if (SessionInfoEscape.IsValid() && SessionInfoEscape->HostAddr.IsValid())
	{
		SessionInfoEscape->HostAddr->SetPort(NetDriverPort);
	}
}

bool FOnlineSessionEscape::IsHost(const FNamedOnlineSession& Session) const
{
	if (EscapeSubsystem->IsDedicated())
	{
		return true;
	...
	IOnlineIdentityPtr IdentityInt = EscapeSubsystem->GetIdentityInterface(); 
...
TSharedPtr<const FUniqueNetId> FOnlineSessionEscape::CreateSessionIdFromString(const FString& SessionIdStr)
{
	TSharedPtr<const FUniqueNetId> SessionId;
	if (!SessionIdStr.IsEmpty())
	{
		SessionId = MakeShared<FUniqueNetIdEscape>(SessionIdStr);
	}
	return SessionId;
}
